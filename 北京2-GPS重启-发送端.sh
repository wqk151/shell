...
if [ $data_line_num -lt 1 ];then
    echo "data is empty"
    ssh -p 52523 api-datatang.chinacloudapp.cn "sh /data/sh/beijingImpoTexi2Kafka.sh 2"
    sleep 60 # wait 60s
    get_data_line_num
    if [ $data_line_num -lt 1 ];then
        echo "data also is empty please wait 10min"
        ssh -p 52523 api-datatang.chinacloudapp.cn "sh /data/sh/beijingImpoTexi2Kafka.sh 600"
     fi
